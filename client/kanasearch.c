/* $Id: kanasearch.c,v 1.1 2001/11/03 22:36:09 gray Exp $
 */
#ifdef HAVE_CONFIG_H
# include <config.h>
#endif
...
XChar2b * get_key_label __PROTO((Keyboard *, int, int));
void setkbdstate __PROTO((Keyboard *, int));
void clearkbdstate __PROTO((Keyboard *));
...
void 
stopCallback(widget, closure, call_data)
    Widget widget;
 ...
 */
void
Showpopupkana(w, client_data, call_data)
    Widget w;
...
void
ksearch_toggles(w, data, call_data)
    Widget w;
    XtPointer data;
    XtPointer call_data;
{
...
void
kana_backspace(w, data, call_data)
    Widget w;
    XtPointer data;
    XtPointer call_data;
{
...
void
state_toggles(w, data, call_data)
    Widget w;
    XtPointer data;
    XtPointer call_data;
{
...
void
switch_mode(w, event, params, num_params)
    Widget w;
    XEvent *event;
    String *params;
    Cardinal *num_params;
{
	...
	warning("switch-mode(): invalid mode: %s", params[0]);
}

/* This is for the kana input window */

void
makeinputwidgets(parent)
    Widget parent;
{
    int i, j, max_kana = 0;
    char namestr[10];
