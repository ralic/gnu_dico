/* $Id: widgets.c,v 1.1 2001/11/03 22:36:26 gray Exp $
 */
#ifdef HAVE_CONFIG_H
# include <config.h>
#endif
...
void
delete_callback(w, event, params, num_params)
    Widget w;
    XEvent *event;
    String *params;
    Cardinal *num_params;
{
    warning("delete_callback() called?!");
...
initfonts()
{
    DEBUG(("LargeKanjiFont \"%s\"", config.LargeKanjiFont));
    DEBUG(("SmallKanjiFont \"%s\"", config.SmallKanjiFont));
    DEBUG(("BushuFont      \"%s\"", config.BushuFont));
    DEBUG(("EnglishFont    \"%s\"", config.EnglishFont));

    largekfont = XLoadQueryFont(display, config.LargeKanjiFont);
    smallkfont = XLoadQueryFont(display, config.SmallKanjiFont);
    bushufont = XLoadQueryFont(display, config.BushuFont);
    englishfont = XLoadQueryFont(display, config.EnglishFont);
    if (largekfont == NULL) {
	die("could not load large kanji font `%s'",
	    config.LargeKanjiFont);
    }
    if (smallkfont == NULL) {
	die("could not load small kanji font `%s'",
	    config.SmallKanjiFont);
    }
    if (bushufont == NULL) {
	error("could not load bushu font `%s'", config.BushuFont);
    }
    if (englishfont == NULL) {
	die("could not load english font `%s'!!!", config.EnglishFont);
    }
}
...
Widget 
CreateNameList(w, cnt, name, multsel, callback, argc, item_args, return_widgets)
    Widget w;
    int cnt;
    String *name;
    Boolean multsel;
    ...
    Widget **return_widgets;
{
    ...
    if (!args) {
	error("%s:%d:memory!", __FILE__, __LINE__);
	return NULL;
    }
    if (argc)
