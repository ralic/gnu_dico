/* $Id: convert.c,v 1.1 2001/11/03 22:35:54 gray Exp $
*/
#ifdef HAVE_CONFIG_H
# include <config.h>
#endif
    ...
    XChar2b *kana;
{
	...
	if (kana->byte1 == 0x21) {
	    ...
	    warning("kanatoromaji found 0x21,%#x", val);
	    continue;

	}
	if (kana->byte1 != 0x24 && kana->byte1 != 0x25
	    && kana->byte1 != 0x0) {
	    warning("0x%x%x found in kanatoromaji", kana->byte1, kana->byte2);
	    kana++;
	    continue;
	}
    ...
    int buflen;
{
	...
	if (src->byte1 == 0)
	    ...
	    warning("romajitoascii() choked at {%#x, %#x} :(",
		    src->byte1, src->byte2);
	    buf[0] = 0;
	    break;
	}
...
void
Handle_romajikana(w, closure, e, cont)
    Widget w;
    XtPointer closure;
    XEvent *e;
    Boolean *cont;
{
	...
	warning("Some other strange event found in for romaji: %d\n", e->type);
	return;
    }
...
*/	
	...
	warning("NULL keysym on kana input???");
	return;
    }
    switch (inbetweenK) {
    case XK_BackSpace:
