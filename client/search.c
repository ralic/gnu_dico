/* $Id: search.c,v 1.1 2001/11/03 22:36:21 gray Exp $
 */
#ifdef HAVE_CONFIG_H
# include <config.h>
#endif
    ...
    Widget top;
{
    ...
    menuform = 
	XtVaCreateManagedWidget("menuForm",
				formWidgetClass,
				top,
				NULL);
	...
	XtVaCreateManagedWidget("connect",
				...
				menu[COMMAND_MENU],
				XtNy, 1,
				XtNcallback, consoleCallbackRec,
				NULL);
	...
	XtVaCreateManagedWidget("edit",
				...
				menu[OPTIONS_MENU],
				XtNy, 1,
				XtNcallback, editOptionsCallbackRec,
				NULL);
	...
	XtVaCreateManagedWidget("save",
				...
				menu[OPTIONS_MENU],
				XtNy, 2,
				XtNcallback, saveOptionsCallbackRec,
				NULL);
	...
	XtVaCreateManagedWidget("license",
				...
				menu[HELP_MENU],
				XtNy, 3,
				XtNcallback, licenseCallbackRec,
				NULL);
}


void
MakeSearchWidget(shell, prev)
    Widget shell;
{
    Widget searchform, w, dictform, buttonform;
 ...
 /*
    dictform =
	XtVaCreateManagedWidget("dictForm",
				formWidgetClass,
				searchform,
    ...
    Widget parent;
{
	...
	XtVaCreateWidget("searchDropLabel",
			 ...
			 XtNjustify, XtJustifyLeft,
			 XtNborderWidth, 0,
			 XtNfromVert, searchwidgets[SEARCH_KANA_W],
			 NULL);
	...
	XtVaCreateWidget("searchDropInput",
			 wcharTextWidgetClass,
			 parent,
			 XtNstring, "\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1\xa1",
			 XtNencoding, XtEeuc,
			 XtNdisplayCaret, False,
			 XtNfromHoriz, searchwidgets[SEARCH_KANA_L],
			 XtNfromVert, searchwidgets[SEARCH_KANA_W],
    ...
    DictEntry *entry;
{
	...
	XtVaGetValues(keynumbers[X_LABEL],
		      XtNstate, &state,
		      NULL);
	if (!state)
	    XtVaSetValues(keynumbers[X_LABEL], XtNsensitive, False, NULL);
	XtVaSetValues(kanji_w, XtNlabel, &blank,
	...
	if (entry->kanji == 0) {
	    ...
	    kanjiptr = (XChar2b *)GetString(entry->kanji);
	}
	XtVaSetValues(kanji_w, XtNlabel, kanjiptr,
		      XtNwidth, KANJIWIDTH, XtNheight, KANJIWIDTH, NULL);

	SetWidgetNumber(searchnumbers[G_INPUT], "%d", (int) entry->grade_level);
	SetWidgetNumber(searchnumbers[F_INPUT], "%d", (int) entry->frequency);
	...
	if (search_widget != keynumbers[B_INPUT])
	    SetWidgetBushu(keynumbers[B_INPUT], entry->bushu, entry->numstrokes);
	...
	if (!state)
	    XtVaSetValues(keynumbers[X_LABEL],
			  XtNsensitive, entry->refcnt > 0,
			  NULL);
	
	kanjiptr = (XChar2b *)GetString(entry->kanji);
	if (kanjiptr != NULL)
	    if (kanjiptr[1].byte1 == 0) {
		kindex = kanjiptr[0].byte1;
		kindex = kindex << 8;
		kindex |= kanjiptr[0].byte2;
	    }
	if (kindex == NOKANJI) {
	    kindex = 0;
	}
	...
	if (config.romaji) {
	    ...
	    char *romajiptr = kanatoromaji((XChar2b *)GetString(entry->pronunciation));
...
int
do_match_string(skey, string, dir)
    char *skey;
    char *string;
    Matchdir dir;
{
...
int
do_match_number(skey, value, dir, closest)
    char *skey;
    unsigned value;
    Matchdir dir;
    Boolean closest;
...
/*ARGSUSED*/
void
UnhighlightSearchWidget(w, data, calldata)
    Widget w;
    XtPointer data;
    XtPointer calldata;
{
	...
	XtVaSetValues(search_widget,
		      ...
		      XtNforeground, default_foreground,
		      NULL);
	search_widget = NULL;
    }
}
    ...
    Widget w;
{
	...
	XtVaSetValues(w,
		      ...
		      XtNforeground, config.highlight.foreground,
		      NULL);
    }
}
    ...
    Matchdir dir;
{
	...
	logmsg(L_WARN, "Where did that input come from??");
	return;
    }
}

void
dict_lookup(w, event, params, num_params)
    Widget w;
    XEvent *event;
    String *params;
    Cardinal *num_params;
...
int
handle_lookup_number(w, dir, skey, fmt_let, closest)
    Widget w;
    Matchdir dir;
...
void
findPrevCallback(w, data, calldata)
    Widget w;
    XtPointer data;
    XtPointer calldata;
{
...
void
findNextCallback(w, data, calldata)
    Widget w;
    XtPointer data;
    XtPointer calldata;
{
