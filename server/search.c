...
alloc_entry_buf()
{
	...
	if (p == 0) {
	    logmsg(L_ERR, "not enough core to allocate %d entries", count);
	    if (!entry_buf) {
		logmsg(L_ERR, "fatally low on core, exiting");
		exit(1);
	    }
    ...
    int len;
{
	...
	if (!string_buffer)
	    logmsg(L_ERR, "not enough core to allocate string buffer");
	else
	...
	if (p) {
	    ...
	    logmsg(L_ERR, "not enough core to reallocate string buffer");
	return p;
    }
    return string_buffer;
}
    ...
    int code;
{
    ...
    if (recno == 0) {
	msg(RC_ERR, "can't find master record");
	return;
    }
    ...
    rc = iseek(iptr, &jis);
    if (rc != SUCCESS) {
	logmsg(L_ERR, "%s:%d:iseek(%#x) returned %d",
	    __FILE__, __LINE__, jis, rc);
	msg(RC_ERR, "can't find cross-references");
    }
    
    recbuf = calloc(refcnt+1, sizeof(*recbuf));
    if (!recbuf) {
	logmsg(L_ERR, "low core");
	msg(RC_ERR, "low core");
	return;
    }
	...
	if (iskip(iptr, 1) != 1) {
	    logmsg(L_ERR, "%s:%d: unexpected end of index", __FILE__, __LINE__);
	    break;
	}
    }
    recbuf[0] = i-1;
